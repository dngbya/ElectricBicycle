<template>
  <div class="app-container" ref="appContainer">
    ...
    >
      ...
      <template v-slot:breakDown="scope">{{ scope.row.number-scope.row.usingNumber-scope.row.freeNumber}}</template>
    ...
    <el-dialog v-model="dialogVisible" :title="title" width="50%">
      ...
      >
        ...
        <el-form-item label="使用中数量" prop="usingNumber">
          <el-input v-model="ruleForm.usingNumber" />
        </el-form-item>
        <el-form-item label="空闲数量" prop="freeNumber">
          <el-input v-model="ruleForm.freeNumber" /> </el-form-item
        ><el-form-item label="图片" prop="avator">
          <el-upload
          ...
          >
            <img v-if="ruleForm.image" :src="ruleForm.image" style="width: 178px; height: 178px" class="avatar" />
...
const changeStatus = (row) => {
    ElMessageBox.confirm(
        `确定要${!row.status ? '禁用' : '启用'} ${row.name} 角色吗？`,
        '温馨提示',
        {
...
const UploadImage=(file)=> {
      ...
      axios.post('http://localhost:8086/upload', fd, {headers: {'Content-Type': 'multipart/form-data'}}).then(response => {
        ...
        // 将图片地址给到表单.
        ruleForm.value.image=response.data.data
...
//获取表格
const getUserList = async (position='') => {
...
const handleClose = async (done: () => void) => {
  ...
  if (title.value === '新增') {
    ...
    const response = await axios.post('http://localhost:8086/charging', ruleForm.value)
    ElMessage.success('添加成功')
  } else if (title.value === '编辑') {
    console.log('编辑===', title.value)
    console.log('ruleForm===', ruleForm)
    const response = await axios.put('http://localhost:8086/charging', ruleForm.value)
...
const batchDelete = () => {
  ...
  ElMessageBox.confirm('你确定要删除选中项吗?', '温馨提示', {
    ...
    .then(() => {
      ...
      // @ts-ignore
      axios.delete(`http://localhost:8086/charging/${ids}`).then((res) => {
        ElMessage({
          type: 'success',
          message: '删除成功',
        })
...
const onSubmit = (val) => {
  ...
  getUserList(val.value.position)
  setTimeout(() => {
    loading.value = false
  }, 500)
}

...
//获取表格
const getUserList = async (position = '',status='') => {
  const response = await axios.get('http://localhost:8086/chargingPile', {
    params: {
      //查询参数
      //page: config.page.value, // 默认值是1   可以挨个点  可以 5 4 1 2
      position, //name其实跟name:name效果是一样的，当key value重名情况下，name就可以缩写
      status
      /* id,
      sex */
    },
...
const handleClose = async (done: () => void) => {
  ...
  if (title.value === '新增') {
    ...
    const response = await axios.post('http://localhost:8086/chargingPile', ruleForm.value)
    ElMessage.success('添加成功')
  } else if (title.value === '编辑') {
    console.log('编辑===', title.value)
    console.log('ruleForm===', ruleForm)
    const response = await axios.put('http://localhost:8086/chargingPile', ruleForm.value)
...
const batchDelete = () => {
  ...
  ElMessageBox.confirm('你确定要删除选中项吗?', '温馨提示', {
    ...
    .then(() => {
      ...
      // @ts-ignore
      axios.delete(`http://localhost:8086/chargingPile/${ids}`).then((res) => {
        ElMessage({
          type: 'success',
          message: '删除成功',
        })
...
const onSubmit = (val) => {
  ...
  getUserList(val.value.position,val.value.status)
  setTimeout(() => {
    loading.value = false
  }, 500)
}
